...
const SessionStats: React.FC<Props> = ({ className }) => {
    ...
    if (isLoading) {
        return (
            <Card className={cn("w-full border dark:border-gray-700 bg-white dark:bg-neutral-900 rounded-[4px] p-4", className)}>
                <CardHeader>
            ...
            <CardContent className="grid gap-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-4">
                    {countryData && Object.values(countryData).map((session) => (
                        <Card key={session.country} className="border dark:border-gray-700 bg-white dark:bg-neutral-800 rounded-[4px] p-4">
